...
REST. For this assignment I've decided to code up the REST calls manually using NSURLSession. For a bigger application which would talk to more endpoints I would consider using AFNetworking, but now adays it's pretty similar to what you get with NSURLSession.
The REST manager class is exposed as a singleton which is clipped to 1 concurrent call. This could be increased in a busier application with some caution. For what we are doing, 1 concurrent call is plenty.
